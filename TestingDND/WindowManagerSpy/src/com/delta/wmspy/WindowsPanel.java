...
public class WindowsPanel implements WMSpyPanel {
    ...
    public void populate() {
        try {
            ...
            Map<Mode, AuxWindow> modesMap = (Map<Mode, AuxWindow>)editorsObject;
            WeakHashMap<AuxWindow, Reference<Mode>> lastActiveModeMap = (WeakHashMap<AuxWindow, Reference<Mode>>)activeObject;
            lm.clear();
            
            for(Mode mode: modesMap.keySet()) {
                AuxWindow window = modesMap.get(mode);
                lm.addElement(mode.getName() + " -> " + (window==null?"NbMainWindow":window.getName()));
            }
            lm.addElement("--------------");
            for(AuxWindow window: lastActiveModeMap.keySet()) {
    ...
    public String getTitle() {
        return "ModesSubModel:mode2AuxWindow";
    }
