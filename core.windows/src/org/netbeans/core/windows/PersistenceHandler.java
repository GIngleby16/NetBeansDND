...
final public class PersistenceHandler implements PersistenceObserver {
    ...
    private ModeImpl createModeFromConfig(ModeConfig mc) {
        ...
        // need to find aux-window by ID (or create?)
        AuxWindowImpl window = (AuxWindowImpl)WindowManagerImpl.getInstance().findAuxWindow(mc.auxWindowID);
...
//        }
    ...
    private WindowManagerConfig getConfig() {
        ...
        wmc.auxWindows = getConfigFromAuxWindow(WindowManagerImpl.getInstance().getAuxWindows());
    ...
    private ModeConfig getConfigFromMode(ModeImpl mode) {
        ...
        AuxWindowImpl auxWindow = WindowManagerImpl.getInstance().getWindowForMode(mode);
        modeCfg.auxWindowID = auxWindow == null ? "" : auxWindow.getName();
        if(DEBUG) {
            debugLog("mode auxWindowID=" + modeCfg.auxWindowID); // NOI18N
        }
    ...
    private AuxWindowConfig[] getConfigFromAuxWindow(Set<AuxWindow> windows) {
        List<AuxWindowConfig> winConfigs = new ArrayList<AuxWindowConfig>(windows.size());
        for(AuxWindow win: windows)
            winConfigs.add(new AuxWindowConfig(win.getName(), win.getBounds()));
        return winConfigs.toArray(new AuxWindowConfig[windows.size()]);
    }
}
