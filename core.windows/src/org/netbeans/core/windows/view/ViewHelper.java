...
import org.netbeans.core.windows.view.AuxWindowStructureAccessorImpl.AuxWindowAccessorImpl;
...
//        wsa.setAuxWindowStructureAccessor(createAuxWindowStructureAccessor(wss.getAuxWindowSnapshot()));
        ...
        Map<AuxWindowSnapshot, ModeSnapshot> maximizedSnapshot = wss.getMaximizedModeSnapshot();
        Map<AuxWindowAccessor, ModeAccessor> maximizedAccessors = new HashMap<AuxWindowAccessor, ModeAccessor>();
        for(AuxWindowSnapshot aws: maximizedSnapshot.keySet()) {
            ModeSnapshot ms = maximizedSnapshot.get(aws);
            ModeAccessor ma = ms == null ? null : msa.findModeAccessor(ms.getName());
            maximizedAccessors.put(new AuxWindowAccessorImpl(aws), ma);
        }
    ...
    //TODO gwi: modified to include AuxSplitRoots
    private static ModeStructureAccessorImpl createModeStructureAccessor(ModeStructureSnapshot mss) {
        Map<AuxWindowSnapshot, WindowModeStructureSnapshot> windows = mss.getWindowModeStructures();
        ...
        // null window will be the main-window, all others are aux-windows
        for(AuxWindowSnapshot windowSnapshot: windows.keySet()) {
            ...
            windowAccessors.put(new AuxWindowAccessorImpl(windowSnapshot), wmsa);  
...
//        return msa;
    ...
    private static Set<SlidingAccessor> createSlidingModeAccessors(AuxWindowSnapshot windowSnapshot, ModeStructureSnapshot.SlidingModeSnapshot[] slidingModeSnapshots) {
     ...
     *
     */
    public static void setSplitWeights(AuxWindowImpl window, SplitAccessor splitAccessor,
