...
import org.netbeans.core.windows.view.ui.AuxWindowComponent;
import org.netbeans.core.windows.view.ui.AuxWindowDialog;
import org.netbeans.core.windows.view.ui.AuxWindowFrame;
...
final class ViewHierarchy {
    ...
    private Map<AuxWindowImpl, AuxWindowComponent> win2Frame = new HashMap<AuxWindowImpl, AuxWindowComponent>();
    private final AuxWindowListener auxWindowListener;
//    private Map<AuxWindowSnapshot, ViewElement> auxCurrentSplitRoot = new HashMap<AuxWindowSnapshot, ViewElement>();
     ...
     */
    private Map<AuxWindowImpl, DesktopImpl> desktopMap = new HashMap<AuxWindowImpl, DesktopImpl>();  
    private Map<AuxWindowImpl, ViewElement> currentSplitRootMap = new HashMap<AuxWindowImpl, ViewElement>(); 
    private Map<AuxWindowImpl, ViewElement> fakeSplitMap = new HashMap<AuxWindowImpl, ViewElement>();
     ...
     */
    private final Map<AuxWindowImpl, Map<SlidingView, SlidingAccessor>> slidingModeViewsMap = new HashMap<AuxWindowImpl, Map<SlidingView, SlidingAccessor>>(10);
    ...
    public ViewHierarchy(Controller controller, WindowDnDManager windowDnDManager) {
        ...
        this.auxWindowListener = new AuxWindowListener(controller, this);
    }
    ...
    public void updateViewHierarchy(ModeStructureAccessor modeStructureAccessor) {
        Map<AuxWindowAccessor, WindowModeStructureAccessor> wmsa = modeStructureAccessor.getWindowModeStructureAccessor();
        ...
        // for each window
        for(AuxWindowAccessor wa: wmsa.keySet()) {
            AuxWindowImpl window = wa.getAuxWindow();
...
//            }
    ...
    public void updateAccessors(ModeStructureAccessor modeStructureAccessor) {
        ...
        Map<AuxWindowAccessor, WindowModeStructureAccessor> windowsAccessor = modeStructureAccessor.getWindowModeStructureAccessor();
    ...
    private ViewElement updateViewForAccessor(AuxWindowImpl window, ElementAccessor patternAccessor) {
...
//            JOptionPane.showMessageDialog(null, "updateViewAccessorFor window=" + window + " pattern=" + patternAccessor);
        ...
        if (view != null) {
            ...
            if (patternAccessor instanceof SplitAccessor) {
                ...
                ModeAccessor ma = (ModeAccessor) patternAccessor;
                ModeView mv;
                if (ma.getState() == Constants.MODE_STATE_JOINED || ma.getState() == Constants.MODE_STATE_AUX) {
    ...
    private void updateSeparateViews(AuxWindowImpl window, ModeAccessor[] separateModeAccessors) {
    ...
    private void updateSlidingViews(AuxWindowImpl window, SlidingAccessor[] slidingModeAccessors) {
...
//            JOptionPane.showMessageDialog(null, "desktop for " + (window==null?"NbMainWindow":window.getName()) + " " + curSv.getSide() + " " + curSv.getTopComponents().size());
    ...
    public Component getSlidingModeComponent(AuxWindowImpl window, String side) {
    ...
    private Component getDesktopComponent(AuxWindowImpl window) {
        return currentSplitRootMap.get(window) == null ? null : getDesktop(window).getDesktopComponent();
    }

    public ViewElement getSplitRootElement(AuxWindowImpl window) {
    ...
    public void setSplitModesVisible(boolean visible) {
        ...
        // for each window 
        for(AuxWindowImpl window: win2Frame.keySet()) {
    ...
    private void setMaximizedViewIntoDesktop(AuxWindowImpl window, ViewElement elem) {
    ...
    private void setSplitRootIntoDesktop(AuxWindowImpl window, ViewElement root) {
        ...
        if(window != null) {
            ...
            if (frame == null) {
                if(window.isDialogRequested()) {
                    frame = new AuxWindowDialog(window, window.getBounds(), controller);
                } else {
                    frame = new AuxWindowFrame(window, window.getBounds(), controller);
                }
                win2Frame.put(window, (AuxWindowComponent)frame);

                // install listeners
                frame.addComponentListener(auxWindowListener);
            }
...
//            debugLog("revalidating..size=" + desktop.getDesktopComponent().getSize() + "innerpane=" + desktop.getInnerPaneDimension());
    ...
    public void updateDesktopTest(ModeAccessor ma, WindowSystemAccessor wsa) {
        ...
        try {
            if (wsa.getEditorAreaState() == Constants.EDITOR_AREA_JOINED) {
                if (maximizedModeView != null) {
                    if(window == null) {
                        setMainWindowDesktop(getDesktopComponent(window));  // NO THIS NEEDS TO SET THE DESKTOP INTO THE APPROPIATE WINDOW (MAIN OR AUX)
                    } else {
                        // put desktop component into aux window?
                    }
    ...
    public void performSlideIn(SlideOperation operation) {
        getDesktop(operation.getAuxWindow()).performSlideIn(operation, getPureEditorAreaBounds(operation.getAuxWindow())); //TODO gwi-window: using null will be main window only
    }

    public void performSlideOut(SlideOperation operation) {
        getDesktop(operation.getAuxWindow()).performSlideOut(operation, getPureEditorAreaBounds(operation.getAuxWindow())); //TODO gwi-window: using null will be main window only
    }

    public void performSlideIntoDesktop(SlideOperation operation) {
        getDesktop(operation.getAuxWindow()).performSlideIntoDesktop(operation, getPureEditorAreaBounds(operation.getAuxWindow())); //TODO gwi-window: using null will be main window only
    }

    public void performSlideIntoEdge(SlideOperation operation) {
        getDesktop(operation.getAuxWindow()).performSlideIntoEdge(operation, getPureEditorAreaBounds(operation.getAuxWindow())); //TODO gwi-window: using null will be main window only
    }

    public void performSlideResize(SlideOperation operation) {
        getDesktop(operation.getAuxWindow()).performSlideResize(operation);
    }
    ...
    private boolean hasEditorAreaVisibleView(AuxWindowImpl window) {
    ...
    private void closeEditorModes(AuxWindowImpl window) {
    ...
    // XXX
    public Rectangle getPureEditorAreaBounds(AuxWindowImpl window) {
    ...
    private EditorView findEditorAreaElement(AuxWindowImpl window) {
        return findEditorViewForElement(getSplitRootElement(window));
    }

    Component getEditorAreaComponent(AuxWindowImpl window) {
    ...
    public String toString() {
        ...
        for(AuxWindowImpl window: currentSplitRootMap.keySet()) {
        ...
        for(AuxWindowImpl window: desktopMap.keySet()) {
        ...
        for(AuxWindowImpl window: fakeSplitMap.keySet()) {
    ...
    private ViewElement getFakeSplitRoot(AuxWindowImpl window) {
    ...
    private DesktopImpl getDesktop(AuxWindowImpl window) {
        synchronized (this) {
    ...
    // NEW -------------------------------------------------------------------
    public void updateAuxWindows(AuxWindowStructureAccessor auxWindowStructureAccessor) {
        Set<AuxWindowImpl> oldFrameNames = new HashSet<AuxWindowImpl>(win2Frame.keySet());
        Set<AuxWindowImpl> newFrameNames = new HashSet<AuxWindowImpl>();

        AuxWindowAccessor[] auxWindowAccessors = auxWindowStructureAccessor.getAuxWindowAccessors();
        for (AuxWindowAccessor winAccessor : auxWindowAccessors) {
            newFrameNames.add(winAccessor.getAuxWindow());
        }
        oldFrameNames.removeAll(newFrameNames);

        // hide closed frames
        for (AuxWindowImpl win : oldFrameNames) {
            AuxWindowComponent window = win2Frame.get(win);
        ...
        for (AuxWindowAccessor winAccessor : auxWindowAccessors) {
            ...
            AuxWindowComponent frame = win2Frame.get(winAccessor.getAuxWindow());
            if (frame == null) {
                AuxWindowImpl win = winAccessor.getAuxWindow();

                frame = new AuxWindowFrame(win, winAccessor.getBounds(), controller);
                win2Frame.put(win, frame);

                // install listeners
                frame.addComponentListener(auxWindowListener);
            }
            if (!frame.isVisible() && winAccessor.getAuxWindow().isVisible()) {
                frame.setVisible(true);
            }
        }
    }
    ...
    public AuxWindowComponent getAuxWindowFrame(AuxWindowImpl window) {
        return win2Frame.get(window);
    }

    //TODO gwi-window: rename getAuxWindowComponent
    public AuxWindowComponent getAuxWindowFrame(String name) {
        // temp! methods using this need to be refactored to use the AuxWindowImpl
        for (AuxWindowImpl win : win2Frame.keySet()) {
     ...
     * Aux window listener.
     */
    private static class AuxWindowListener extends ComponentAdapter
            implements WindowStateListener {

        private final Controller controller;
        private final ViewHierarchy hierarchy;

        public AuxWindowListener(Controller controller, ViewHierarchy hierarchy) {
            this.controller = controller;
            this.hierarchy = hierarchy;
        }
        ...
        public void componentResized(ComponentEvent evt) {
            ...
            AuxWindow window = ((AuxWindowComponent) frame).getAuxWindow();
            controller.userResizedAuxWindow((AuxWindowImpl)window, evt.getComponent().getBounds());
        }
        ...
        public void componentMoved(ComponentEvent evt) {
            Window frame = (Window) evt.getComponent();
            AuxWindow window = ((AuxWindowComponent) frame).getAuxWindow();
            controller.userMovedAuxWindow((AuxWindowImpl)window, evt.getComponent().getBounds());
        }
...
//            }
    ...
    } // End of aux window listener.

    public void hideAuxWindows() {
        for (AuxWindowComponent win : win2Frame.values()) {
            win.setVisible(false);
        }
    }

    public void openZOrderWindows() {
        Set<AuxWindow> windows = WindowManagerImpl.getInstance().getAuxWindows();
        ...
        for (String id : ZOrderManager.getInstance().getZOrder()) {
            if ("NbMainWindow".equals(id)) {
                ...
                for (AuxWindow window : windows) {
                    AuxWindowImpl win = (AuxWindowImpl) window;
                    if (win.getName().equals(id) && win.isVisible()) {
                        AuxWindowFrame frame = (AuxWindowFrame)AuxWindowTracker.getInstance().toWindow(win);
                        frame.setVisible(true);
                    }
                }
            }
    ...
    public Set<Component> getAuxFrames() {
        Set<Component> s = new HashSet<Component>();
        for (AuxWindowComponent w : win2Frame.values()) {
            s.add((Component)w);
        }
    ...
    public void removeAuxWindowImpl(AuxWindowImpl window) {
