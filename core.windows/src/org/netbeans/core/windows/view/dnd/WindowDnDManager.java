...
implements DropTargetGlassPane.Observer, DropTargetGlassPane.Informer {
    ...
    private final Set<Component> auxFrames = new WeakSet<Component>(4);
    ...
    private static WeakHashMap<AuxWindowImpl, WeakReference<CenterPanelDroppable>> centerDropMap = 
            new WeakHashMap<AuxWindowImpl, WeakReference<CenterPanelDroppable>>();
    ...
    TopComponentDraggable startingTransfer) {
        ...
        Set<Component> addedAuxFrames = new HashSet<Component>();
        ...
        // can we add aux frames?
        for(Component w: viewAccessor.getAuxFrames()) {
            if(w != null) {
                addedAuxFrames.add(w);
            }
        }
        ...
        if(!addedAuxFrames.isEmpty()) {
            synchronized(auxFrames) {
                auxFrames.addAll(addedAuxFrames);
            }
        }        
    ...
    public Set<Component> getAuxFrames() {
        synchronized(auxFrames) {
            return new HashSet<Component>(auxFrames);
        }
    }
    ...
    /** Checks whether the point is inside one of aux window,
     * i.e. separated modes, droppable area. The point is relative to screen. */
    private boolean isInAuxFrameDroppable(Set<Component> auxFrames, Point location, TopComponentDraggable transfer) {
        return findAuxFrameDroppable(auxFrames, location, transfer) != null;
    }
     ...
     * by screen location. */
    private boolean isInAnyWindowDroppable(AuxWindowImpl auxWindow, Point location, TopComponentDraggable transfer) {
        return findAnyWindowDroppable(auxWindow, location, transfer) != null;
    }
    ...
    Set<Component> floatingFrames, Point location, TopComponentDraggable transfer) {
        ...
        if (Boolean.getBoolean("netbeans.winsys.auxwindows")) {
            droppable = findAuxFrameDroppable(floatingFrames, location, transfer);
            if(droppable != null) {
                return droppable;
            } 
        }
...
//        }
     ...
     * Helper method. */
    private TopComponentDroppable findAnyWindowDroppable(AuxWindowImpl auxWindow, Point location, TopComponentDraggable transfer) {
        
        //JFrame mainWindow = (JFrame)WindowManagerImpl.getInstance().getMainWindow();
        RootPaneContainer theWindow = (RootPaneContainer)AuxWindowTracker.getInstance().toWindow(auxWindow);  // one day will need to support JDialog too
        ...
        if( transfer.isSlidingEnabled() ) {
            ...
            TopComponentDroppable droppable = findSlideDroppable(viewAccessor.getSlidingModeComponent(auxWindow, Constants.LEFT));
            if (droppable != null) {
            ...
            droppable = findSlideDroppable(viewAccessor.getSlidingModeComponent(auxWindow, Constants.RIGHT));
            if (droppable != null) {
            ...
            droppable = findSlideDroppable(viewAccessor.getSlidingModeComponent(auxWindow, Constants.BOTTOM));
            if (droppable != null) {
            ...
            droppable = findSlideDroppable(viewAccessor.getSlidingModeComponent(auxWindow, Constants.TOP));
            if (droppable != null) {
        ...
        lastSlideDroppable = null;
        if (isNearEditorEdge(auxWindow, location, viewAccessor, transfer.getKind())) {
            return getEditorAreaDroppable(auxWindow);
        }
        if (isNearEdge(auxWindow, location, viewAccessor)) {
            return getCenterPanelDroppable(auxWindow);
        }
...
private TopComponentDroppable findAuxFrameDroppable(
    Set<Component> auxFrames, Point location, TopComponentDraggable transfer) {
        for(Component comp: auxFrames) {
            if(comp instanceof AuxWindowComponent) {
                AuxWindowComponent frame = (AuxWindowComponent)comp;
                AuxWindowImpl auxWindow = AuxWindowTracker.getInstance().toAuxWindow((Window)frame);
                TopComponentDroppable tcd = findAnyWindowDroppable(auxWindow, location, transfer);
                if(tcd != null) {
                    return tcd;
                }
            }
...
//            }
        ...
        if( res instanceof ModeComponent && transfer.getKind() != Constants.MODE_KIND_EDITOR && ((ModeComponent)res).getKind() == Constants.MODE_KIND_EDITOR ) {
            ...
            if(w != null) {
                if(w instanceof AuxWindowComponent)
                    window = (AuxWindowImpl)((AuxWindowComponent)w).getAuxWindow();
            }
     ...
     * In that case is needed also to provide a drop. */
    static boolean isNearEditorEdge(AuxWindowImpl window, Point location, ViewAccessor viewAccessor, int kind) {
    ...
    /** Indicates whether the cursor is around center panel of main window.
     * In that case is needed also to provide a drop. */
    static boolean isNearEdge(AuxWindowImpl window, Point location, ViewAccessor viewAccessor) {
        //Component desktop = MainWindow.getInstance().getDesktop();
        Component desktop = AuxWindowTracker.getInstance().getDesktopComponent(window);
        if(desktop == null) {
            return false;
        }
        Point p = new Point(location);
    ...
    TopComponentDroppable getCenterPanelDroppable(AuxWindowImpl auxWindow) {
        ...
        Reference ref = centerDropMap.get(auxWindow);
        if(droppable == null || ref == null) {             
            droppable = new CenterPanelDroppable(auxWindow);
            centerDropMap.put(auxWindow, new WeakReference<CenterPanelDroppable>(droppable));
        }
    ...
    private TopComponentDroppable getEditorAreaDroppable(AuxWindowImpl window) {
...
//        return droppable;
    ...
    TopComponentDroppable droppable, TopComponentDraggable draggable, Point location) {
        ...
        AuxWindowImpl win = null;
        if(endFrame instanceof AuxWindowComponent) {
            win = (AuxWindowImpl)((AuxWindowComponent)endFrame).getAuxWindow();
        }
    ...
    private static class MotionListener implements DragSourceMotionListener {
        ...
        public void dragMouseMoved(DragSourceDragEvent evt) {
            ...
            boolean isInAuxDroppable
                    = windowDnDManager.isInAuxFrameDroppable(windowDnDManager.getAuxFrames(), location, windowDnDManager.startingTransfer);
            ...
            if(isInMainDroppable || isInFrameDroppable || isAroundCenterPanel || isInAuxDroppable) {
                Set<Component> searchMe = new HashSet<Component>();
                searchMe.addAll(windowDnDManager.getFloatingFrames());
                searchMe.addAll(windowDnDManager.getAuxFrames());
                TopComponentDroppable droppable 
    ...
    private class CenterPanelDroppable implements TopComponentDroppable {
        private AuxWindowImpl auxWindow;
        private Component desktop;
        
        public CenterPanelDroppable(AuxWindowImpl auxWindow) {
            this.auxWindow = auxWindow;
            this.desktop = AuxWindowTracker.getInstance().getDesktopComponent(auxWindow);
        }
        ...
        public Object getConstraintForLocation(Point p) {
            Rectangle bounds = getDropComponent().getBounds();
            Component leftSlide = viewAccessor.getSlidingModeComponent(auxWindow, Constants.LEFT);
            Component rightSlide = viewAccessor.getSlidingModeComponent(auxWindow, Constants.RIGHT);
            Component bottomSlide = viewAccessor.getSlidingModeComponent(auxWindow, Constants.BOTTOM);
            Component topSlide = viewAccessor.getSlidingModeComponent(auxWindow, Constants.TOP);
    ...
    private class EditorAreaDroppable implements TopComponentDroppable {
        AuxWindowImpl window;
        
        public EditorAreaDroppable(AuxWindowImpl window) {
            this.window = window;
        }
