    ...
    public void changeGUI(ViewEvent[] viewEvents, WindowSystemSnapshot snapshot) {
        ...
        for(int i = 0; i < viewEvents.length; i++) {
            ...
            if(changeType == CHANGE_MAIN_WINDOW_BOUNDS_JOINED_CHANGED) {
                ...
                Map<AuxWindowAccessor, ModeAccessor> map = wsa.getMaximizedModeAccessor();
                for(AuxWindowAccessor awa: map.keySet()) {
                    // for each window
...
//                hierarchy.updateDesktop(wsa);
            ...
            } else if (changeType == View.CHANGE_AUX_WINDOW_ADDED) {
                if(DEBUG) {
                    debugLog("Aux Window added"); //NOI18N
                }
//                hierarchy.updateAuxWindows(wsa.getAuxWindowStructureAccessor());
//                System.out.println("ACTIVE_MODE=" + wsa.getActiveModeAccessor());
//                hierarchy.activateMode(wsa.getActiveModeAccessor());
            } else if (changeType == View.CHANGE_AUX_WINDOW_REMOVED) {
                if(DEBUG) {
                    debugLog("Aux Window removed"); //NOI18N
                }
                AuxWindowImpl window = (AuxWindowImpl)viewEvent.getNewValue();
                hierarchy.removeAuxWindowImpl(window);
//                hierarchy.updateAuxWindows(wsa.getAuxWindowStructureAccessor());
//                System.out.println("ACTIVE_MODE=" + wsa.getActiveModeAccessor());
//                hierarchy.activateMode(wsa.getActiveModeAccessor());
            } else if (changeType == View.CHANGE_AUX_WINDOW_BOUNDS_CHANGED) {
                if(DEBUG) {
                    debugLog("Aux Window bounds changed"); //NOI18N
                }
                AuxWindowImpl window = (AuxWindowImpl)viewEvent.getSource();
                Rectangle bounds = (Rectangle)viewEvent.getNewValue();
                AuxWindowComponent frame = hierarchy.getAuxWindowFrame(window);
...
//        }, AWTEvent.FOCUS_EVENT_MASK);
        ...
        // When to show aux-windows??? (according to zorder)
...
//                updateSeparateBoundsForView(hierarchy.getSplitRootElement());
         ...
         * HOPEFULLY THIS WILL STOP ONCE THE AUX-WINDOWS HAVE MODES AND THE CORRECT MODE IS ACTIVATED!
         */
        hierarchy.activateMode(wsa.getActiveModeAccessor());
    ...
    private void hideWindowSystem() {
        ...
        // hide all aux-windows!
        hierarchy.hideAuxWindows();
    ...
    @Override
    public void userMovedSplit(AuxWindowImpl window, SplitView splitView, ViewElement[] childrenViews, double[] splitWeights) {
        if(DEBUG) {
...
//        updateSeparateBoundsForView(splitView);
    ...
    // DnD
    @Override
    public void userDroppedTopComponents(AuxWindowImpl window, ModeView modeView, TopComponentDraggable draggable) {
        if(DEBUG) {
    ...
    @Override
    public void userDroppedTopComponents(AuxWindowImpl window, ModeView modeView, TopComponentDraggable draggable, int index) {
        if(DEBUG) {
    ...
    @Override
    public void userDroppedTopComponents(AuxWindowImpl window, ModeView modeView, TopComponentDraggable draggable, String side) {
        if(DEBUG) {
    ...
    @Override
    public void userEmptiedAuxWindow(AuxWindowImpl window) {
        controllerHandler.destroyAuxWindow(window);
    }
    
    
    
    public void userClosingAuxWindow(AuxWindowImpl window) {
        controllerHandler.userClosedAuxWindow(window);
        hierarchy.removeAuxWindowImpl(window);
    }
    
    @Override
    public void userMovedAuxWindow(AuxWindowImpl window, Rectangle bounds) {
        if(window == null)
            return; // ignore main-window

        if(DEBUG) {
            debugLog("User moved aux window");
        }
                
        // Ignore when window is maximixed
        AuxWindowComponent comp = hierarchy.getAuxWindowFrame(window);
        if(comp instanceof AuxWindowFrame) {
            AuxWindowFrame frame = (AuxWindowFrame)comp;
            if(frame.getExtendedState() == Frame.MAXIMIZED_BOTH) {
                return;
            }
        } 
        controllerHandler.userResizedAuxWindow(window, bounds); // not a typo userResizedAuxWindow
    }
    ...
    public void userResizedAuxWindow(AuxWindowImpl window, Rectangle bounds) {
        ...
        if(DEBUG) {
            debugLog("User resized aux window");
        }
