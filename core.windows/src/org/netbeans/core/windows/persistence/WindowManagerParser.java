...
public class WindowManagerParser {
    ...
    private List<AuxWindow> auxWindows = new ArrayList<AuxWindow>();
...
//                    System.out.println("WindowManagerParser.readData "+(System.currentTimeMillis()-time));
        ...
        public void startElement (String nameSpace, String name, String qname, Attributes attrs) throws SAXException {
            if ("windowmanager".equals(qname)) { // NOI18N
                ...
                } else if ("aux-window".equals(qname)) { // NOI18N
                    handleAuxWindow(attrs);
        ...
        /** Reads element "aux-window" and updates window manager config content */
        private void handleAuxWindow (Attributes attrs) {
            // TODO gwi-p
            ...
            // false will always create an AuxWindowFrame not AuxWindowDialog
            AuxWindow auxWindow = WindowManagerImpl.getInstance().createAuxWindow(attrs.getValue("name"), bounds, false); 
            auxWindows.add(auxWindow);
        }
        ...
        private void appendAuxWindow (WindowManagerConfig wmc, StringBuffer buff) {
            // save name of each aux-window
            AuxWindowConfig[] auxWindows = wmc.auxWindows;
            if(auxWindows != null) {
                for(AuxWindowConfig win: auxWindows) {
                    Rectangle bounds = win.getBounds();
                    buff.append("    <aux-window name=\"" + win.getName() 
                            + "\" x=\"" + bounds.x 
                            + "\" y=\"" + bounds.y 
                            + "\" width=\"" + bounds.width 
